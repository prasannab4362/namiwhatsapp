        ...
        <div className="flex flex-col gap-3">
          {buttons.map((b, i) => (
            <div
              key={i}
              className={cn(
                "grid grid-cols-1 gap-2 rounded-md border border-slate-200 bg-slate-800/40 p-3",
                showAdvanced
    ...
    <>
      ...
      <div className="mt-2">
        ...
        {sections.map((section, sIdx) => (
          <div
            key={sIdx}
            className="mb-3 rounded-md border border-slate-200 bg-slate-800/40 p-3"
          >
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <div>
          <label className="mb-1 block text-xs text-slate-600">If</label>
          <Select
            value={subject}
            onValueChange={(v) =>
      ...
      >
        <div>
          <label className="mb-1 block text-xs text-slate-600">Operator</label>
          <Select
            value={operator}
            onValueChange={(v) =>
        ...
        {showValue && (
          <div>
            <label className="mb-1 block text-xs text-slate-600">Value</label>
            <Input
              value={cfg.value ?? ""}
...
function SetTagForm({
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs text-slate-600">Action</label>
          <Select
            value={cfg.mode ?? "add"}
            onValueChange={(v) =>
        ...
        <div>
          <label className="mb-1 block text-xs text-slate-600">Tag</label>
          {tags.length > 0 ? (
            <Select
              value={cfg.tag_id ?? ""}
...
function SendMediaForm({
  ...
  return (
    <>
      <div>
        <label className="mb-1 block text-xs text-slate-600">Media type</label>
        <Select
          value={mediaType}
          onValueChange={(v) => {
      ...
      <div>
        ...
        {cfg.media_url ? (
          <div className="flex items-center gap-2 rounded-md border border-slate-300 bg-slate-100 px-3 py-2 text-xs">
            <Paperclip className="h-3.5 w-3.5 shrink-0 text-cyan-400" />
            <a
              href={cfg.media_url}
              target="_blank"
              rel="noopener noreferrer"
              className="min-w-0 flex-1 truncate text-slate-800 hover:text-cyan-300"
            ...
            <button
              type="button"
              onClick={handleClear}
              className="rounded p-1 text-slate-600 hover:bg-slate-700 hover:text-slate-800"
          ...
          <button
            ...
            disabled={uploading}
            className="flex w-full items-center justify-center gap-2 rounded-md border border-dashed border-slate-300 bg-white px-3 py-4 text-xs text-slate-600 transition-colors hover:border-slate-600 hover:bg-slate-100 hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {uploading ? (
              <>
