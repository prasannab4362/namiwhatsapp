...
export function ImportModal({ open, onOpenChange, onImported }: ImportModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="bg-white border-slate-300 text-slate-800 sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-slate-900">Import Contacts</DialogTitle>
        ...
        <div className="space-y-4">
          ...
          <div
            ...
            className="flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-300 p-6 cursor-pointer hover:border-primary/50 transition-colors"
          >
            {file ? (
              <>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-100">
                      <th className="px-3 py-1.5 text-left text-slate-600 font-medium">Phone</th>
                      <th className="px-3 py-1.5 text-left text-slate-600 font-medium">Name</th>
                      <th className="px-3 py-1.5 text-left text-slate-600 font-medium">Email</th>
                      <th className="px-3 py-1.5 text-left text-slate-600 font-medium">Company</th>
                    </tr>
                  </thead>
                  <tbody>
                    {preview.map((row, i) => (
                      <tr key={i} className="border-t border-slate-700/50">
                        <td className="px-3 py-1.5 text-slate-700">{row.phone}</td>
          ...
          {/* Results */}
          {result && (
            <div className="rounded-lg border border-slate-300 p-4 space-y-2">
