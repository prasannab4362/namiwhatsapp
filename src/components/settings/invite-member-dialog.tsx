...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Nami CRM account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Nami CRM account"
    // isn't descriptive enough.
    const accountName = result?.accountName ?? 'our Nami CRM account';
    const message = `Join ${accountName} on Nami CRM using this link (valid for ${result?.expiresInDays} days): ${url}`;
